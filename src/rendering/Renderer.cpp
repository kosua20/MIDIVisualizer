  ...
  if (_state.showBlur) {
    ...
    if (_state.showParticles) {
      ...
      _scene->drawParticles(_timer, invSizeB, _state.particles.color,
                            _state.particles.scale, _state.particles.texs,
    ...
    if (_state.showParticles) {
      if (ImGui::CollapsingHeader("Particles##HEADER")) {
        ...
        if (ImGui::Button("Default image")) {
          ...
          _state.particles.texs =
              std::vector<GLuint>(PARTICLES_TEXTURE_COUNT, blankID);
        ...
        if (ImGui::Button("Load images...")) {
          ...
          nfdresult_t result =
              NFD_OpenDialogMultiple("png;jpg,jpeg;", NULL, &outPaths);
        ...
        if (ImGui::IsItemHovered()) {
          ...
          ImGui::TextUnformatted(
              "You can select up to 4 images (PNG or JPEG). They should be "
              "square and B&W, where black is transparent, white opaque.");
          ImGui::PopTextWrapPos();
          ImGui::EndTooltip();
        }
